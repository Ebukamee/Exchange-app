    ...
    user: {
      create: {
        before: async (user) => {
          ...
          console.log("[REFERRAL DEBUG] user object keys:", Object.keys(user));
          console.log("[REFERRAL DEBUG] referral_code_used value:", JSON.stringify(referral_code_used));
          if (referral_code_used) {
            referral_code_used = referral_code_used.trim().toUpperCase();
            console.log("[REFERRAL DEBUG] looking up code:", referral_code_used);
            const { rows } = await pool.query(
              'SELECT id, referral_code FROM "user" WHERE referral_code = $1',
              [referral_code_used]
            );
            console.log("[REFERRAL DEBUG] query result rows:", rows.length, rows);
            if (!rows.length) {
              // Double check — list all referral codes to compare
              const all = await pool.query('SELECT referral_code FROM "user" WHERE referral_code IS NOT NULL LIMIT 20');
              console.log("[REFERRAL DEBUG] existing codes in DB:", all.rows.map(r => r.referral_code));
