...
export async function getBillerProducts(billerCode) {
  if (!billerCode) {
    console.warn("[Monnify Debug] getBillerProducts called without billerCode");
    return [];
  }
  const data = await monnifyFetch(`/api/v1/vas/bills-payment/biller-products?biller_code=${encodeURIComponent(billerCode)}`);
  return normalizeList(data);
}
