...
export async function getBillers(categoryCode) {
  const data = await monnifyFetch(`/api/v1/vas/bills-payment/billers?categoryCode=${categoryCode}`);
  return data?.responseBody ?? [];
}

// Step 3: Get products for a biller
export async function getBillerProducts(billerCode) {
  const data = await monnifyFetch(`/api/v1/vas/bills-payment/biller-products?billerCode=${billerCode}`);
  return data?.responseBody ?? [];
}

// Step 4: Validate customer
export async function validateCustomer({ productCode, customerId }) {
  const data = await monnifyFetch("/api/v1/vas/bills-payment/validate", {
    method: "POST",
...
export async function requeryBill(vendReference) {
  const data = await monnifyFetch(`/api/v1/vas/bills-payment/requery?vendReference=${vendReference}`);
  return data?.responseBody ?? null;
}
